...
- **Current Milestone**: Round 3 — Prod Drift Purge (2026-09-06) — **COMPLETE & VERIFIED**.
- **Round 3 (2026-09-06)** — closes all HIGH findings from the Genspark review (H-1/H-2/H-3) + M-1/M-2/L-4 (plan: `docs/superpowers/plans/2026-09-06-prod-drift-purge.md`, implemented by agy, verified + landed by orchestrator):
  - **Migration 0085** (`venues_data_rescue`): STOP-GATE data check found 2 live rows in prod's drift `venues` table (الكنيسة الكبرى، قاعة المناسبات) with `venues_resources` empty — rescued into `venues_resources` (existence-guarded; capacity column intentionally not carried — 0073 has no such column). No data lost.
  - **Migration 0086** (`prod_drift_purge`): reifies `v_services` (prod-only view the mobile booking home reads; security_invoker=true) + repo's `complaints_admin_assign` policy (0005 never physically ran on prod); drops ALL remaining prod drift — 17 functions (incl. both bigint-keyed `admin_record_cash_payment` overloads; the repo's uuid-keyed one kept), 5 payments drift columns + index, 10 tables, 5 enums, drifted `complaints assign write` policy; pins hardened `search_path` on 19 SECURITY DEFINER functions (H-3). First push attempt rolled back with 2BP01 (`set_updated_at` pinned by triggers on the drift tables) — fixed by moving its drop after the table drops; 0086 amended before remote recording.
  - **Code fixes**: analytics-export CSV response uses dynamic `getCorsHeaders(req)` (M-1); diagnostic-engine alert phone via `DIAGNOSTIC_ALERT_PHONE` env + dispatcher-compatible `params` payload (M-2 — alert could never render before); phone PII scrubbed from `workflows/diagnostic_audit_loop.md`; `offline-sync` removed from systemPatterns diagram (L-4).
  - **Prod verification**: migrations 0085+0086 recorded; drift query confirms 10 tables gone, v_services present, 2 rescued venue rows, payments drift columns gone, complaints policy swapped; cron shows only the 2 legitimate jobs; both changed functions redeployed; **smoke 7/7 PASS** (incl. new v_services check). Local gates: replay 86/86, pgTAP 74/74, deno 78/78.
  - **Accepted residual drift**: prod `booking_status` enum has 4 extra labels (SUBMITTED, REJECTED, PARTIALLY_PAID, PAID) — PG enum labels cannot be dropped. Pre-existing phone literal in `supabase/config.toml` test-OTP fixture (local dev only) flagged for later cleanup.
...
- **Prod drift resolved (2026-09-05)**: the 9 legacy edge functions (paymob-checkout/refund/webhook, reconcile-payments, complaints-decrypt/encrypt, whatsapp-sender, youtube-expiry, offline-sync) were deleted from prod with user approval — `functions list` now shows exactly the 4 repo functions.
...
- **Pending**: `DIAGNOSTIC_ALERT_PHONE` secret on prod diagnostic-engine (needs user-supplied value; without it the security alert is skipped by design).
- **Next Immediate Step**:
  1. Real credentials: WhatsApp (Meta) token/phone ID/webhook secret, FCM service account — needed for notifications to actually send.
  2. Admin PWA hosting decision + Android release keystore before public distribution.

...
void main() {
  testWidgets('home route renders HomeHubScreen', (tester) async {
    ...
    final router = buildRouter(db: fakeDb, isUserLoggedIn: () => true);
  ...
  testWidgets('unauthenticated user stays on home (no redirect to /login)', (
    tester,
  ) async {
    final fakeDb = FakeAppSupabase();
    final router = buildRouter(db: fakeDb, isUserLoggedIn: () => false);
  ...
  testWidgets('payment-proof route renders PaymentProofScreen', (tester) async {
    final fakeDb = FakeAppSupabase();
    final router = buildRouter(db: fakeDb, isUserLoggedIn: () => true);

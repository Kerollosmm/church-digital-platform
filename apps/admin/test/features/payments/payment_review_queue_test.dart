...
void main() {
  group('PaymentsAdminRepository — Payment Proof Review', () {
    test(
      'listPendingProofs returns list of pending proofs from payment_proofs table',
      () async {
    ...
    test(
      'approveProof calls approve_payment_proof RPC and returns Right(null)',
      () async {
        ...
        final repo = PaymentsAdminRepository(mock.build());
        final res = await repo.approveProof(
          101,
          collectorNote: 'Collected note',
        );
    ...
    test(
      'rejectProof calls reject_payment_proof RPC with reasonCode and returns Right(null)',
      () async {
        Map<String, dynamic>? calledArgs;
        final mock = MockSupabase(
          rpc: {
  ...
  group('PaymentReviewQueueScreen Widget Tests', () {
    testWidgets('renders pending proofs and triggers approve action', (
      tester,
    ) async {
      ...
      await tester.pumpWidget(
        MaterialApp(home: PaymentReviewQueueScreen(repo: repo)),
      );
    ...
    testWidgets(
      'renders reject dialog with reason picker and triggers reject action',
      (tester) async {
        ...
        // Tap confirm reject
        final confirmRejectBtn = find.widgetWithText(
          ElevatedButton,
          'تأكيد الرفض',
        );
    ...
    testWidgets('shows empty state when no pending proofs exist', (
      tester,
    ) async {
      ...
      expect(
        find.text('لا توجد إثباتات دفع قيد المراجعة حالياً'),
        findsOneWidget,
      );
    });
  });

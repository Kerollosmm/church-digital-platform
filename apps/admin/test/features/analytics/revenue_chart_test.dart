...
void main() {
  testWidgets('RevenueChartWidget queries v_analytics_payments and renders PieChart with status breakdown and legend', (tester) async {
    final fakeDb = FakeSupabase({
      'v_analytics_payments': [
        {'month': '2026-08-01', 'total_paid': 15000.0, 'total_refunded': 1200.0, 'count_paid': 50},
      ],
    });
  ...
  testWidgets('RevenueChartWidget renders empty state message when no data available', (tester) async {
    final fakeDb = FakeSupabase({
      'v_analytics_payments': [],
    });
  ...
  testWidgets('RevenueChartWidget displays error message on query failure', (tester) async {
    await tester.pumpWidget(
      MaterialApp(
        home: Scaffold(
          body: RevenueChartWidget(
  ...
  testWidgets('ExportReportButton invokes analytics-export Edge Function with GET method and report parameter', (tester) async {
    final fakeDb = FakeSupabase({});

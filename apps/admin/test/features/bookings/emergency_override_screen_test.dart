...
import 'package:admin/features/bookings/emergency_override_repository.dart';
import 'package:admin/features/bookings/emergency_override_screen.dart';
import '../../helpers/mock_supabase.dart';

void main() {
  testWidgets(
    'EmergencyOverrideScreen renders selectors, refund toggle, and calls emergency_override RPC on submit',
    (tester) async {
      final mock = MockSupabase(
        ...
        rpc: {'emergency_override': (args) async => null},
      );

      await tester.pumpWidget(
        MaterialApp(
          home: EmergencyOverrideScreen(
            repo: EmergencyOverrideRepository(mock.build()),
          ),
        ),
      );
      ...
      expect(
        mock.requestLog.any((p) => p.endsWith('/rpc/emergency_override')),
        isTrue,
      );
    },
  );

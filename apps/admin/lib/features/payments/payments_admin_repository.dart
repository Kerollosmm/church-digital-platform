...
class PaymentsAdminRepository {
  ...
  Future<Either<Failure, List<PaymentProofReview>>> listPendingProofs() async {
    try {
      final rows = ((await _db
          ...
          .map((r) => PaymentProofReview.fromJson(Map<String, dynamic>.from(r as Map)))
          .toList();
      return Right(rows);
    } catch (e) {
      return Left(_mapFailure(e));
  ...
  Future<Either<Failure, void>> approveProof(int proofId, {String? collectorNote}) async {
    try {
      await _db.rpc('approve_payment_proof', params: {
        'p_proof_id': proofId,
  ...
  Future<Either<Failure, void>> rejectProof(int proofId, String reasonCode) async {
    try {
      await _db.rpc('reject_payment_proof', params: {
        'p_proof_id': proofId,
  ...
  Future<Either<Failure, List<PayoutChannel>>> listPayoutChannels() async {
    try {
      final rows = ((await _db
          ...
          .map((r) => PayoutChannel.fromJson(Map<String, dynamic>.from(r as Map)))
          .toList();
      return Right(rows);
    } catch (e) {
      return Left(_mapFailure(e));
  ...
  Future<Either<Failure, void>> upsertPayoutChannel(PayoutChannel c) async {
    try {
      await _db.from('payout_channels').update({
  ...
  Future<Either<Failure, void>> markCashReceived(
    ...
    try {
      await _db.rpc('mark_cash_received', params: {
  ...
  Failure _mapFailure(Object e) {
    if (e is PostgrestException) {
      if (e.code == '42501') {
        return const Failure(code: 'FORBIDDEN', message: 'ليس لديك صلاحية لتنفيذ هذا الإجراء');
      }
      if (e.code == '28000') {
        return const Failure(code: 'UNAUTHORIZED', message: 'يرجى تسجيل الدخول أولاً');
      }

-- 0035_concurrency_hardening.sql
-- High-concurrency atomic decrement, schedule exclusion, RLS optimization, and Realtime pub/sub triggers
...
ALTER TABLE public.service_slots
  DROP CONSTRAINT IF EXISTS check_remaining_capacity_non_negative,
  ADD CONSTRAINT check_remaining_capacity_non_negative CHECK (remaining_capacity >= 0);

ALTER TABLE public.service_slots
  DROP CONSTRAINT IF EXISTS check_schedule_valid,
...
GRANT EXECUTE ON FUNCTION public.fn_book_slot_atomic(BIGINT, INT, BOOLEAN, UUID) TO authenticated, service_role;
...
-- 5. Realtime Depletion Broadcast Trigger & Publication Drop
DO $$
BEGIN
  IF EXISTS (

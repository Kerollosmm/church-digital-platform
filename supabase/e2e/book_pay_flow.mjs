...
// NOTE: Inlined copy of hmacFields serializer from functions/_shared/paymob.ts
// to keep this script runnable on plain Node.js without requiring TypeScript runtime loaders.
function hmacFields(t) {
  const o = t.order || {};
  const s = t.source_data || {};
...
const { data: user } = await service
  ...
  .single();
const { data: booking, error: bookErr } = await service.rpc("book_slot", {
  p_slot_id: Number(process.env.TEST_SLOT_ID),
  p_opt_in: true,
});
...
assert(
  checkout.checkout_url.includes("accept.paymob.com"),
  "no checkout url",
);
...
const pay = (
  await service
    .from("payments")
    .select("id")
    .eq("merchant_order_id", String(checkout.payment_id))
    .single()
).data;
const txn = {
  id: 99001,
  ...
  is_voided: false,
  order: { id: 501, merchant_order_id: String(pay.id) },
  owner: 1,
  pending: false,
...
const hookRes = await fetch(
  `${url}/functions/v1/paymob-webhook?hmac=${signature}`,
  {
    method: "POST",
...
const after = (
  await service.from("bookings").select("status").eq("id", booking.id).single()
).data;
assert(
  after.status === "AWAITING_CALL",
  `expected AWAITING_CALL, got ${after.status}`,
);
...
const { data: outbox } = await service
  .from("event_outbox")
  .select("handler_type, payload, status")
  .eq("payload->>phone", user.phone)
  .order("created_at", { ascending: true });
const names = outbox.map((r) => r.payload.template_name);
assert(
  names.includes("booking_payment_received"),
  "booking_payment_received missing",
);
assert(names.includes("booking_confirmed"), "booking_confirmed missing");
assert(
  outbox.every(
    (r) => r.handler_type === "WHATSAPP" && r.status === "PENDING",
  ),
  "events PENDING until dispatcher runs",
);

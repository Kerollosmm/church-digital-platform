...
- **`_shared/http.ts`**: Identity verification and response serialization seam.
  ...
  - `respond(status, codeOrBody, message?, extra?, originOrReq?)`: Unified response and error emitter. Injects secure, origin-validated CORS headers (`Access-Control-Allow-Origin: <allowed_origin>`, `Access-Control-Allow-Headers: ...`, `Vary: Origin`) on every response and formats errors conforming to `{"error": CODE, "message": string?}` (`UNAUTHORIZED`, `FORBIDDEN`, `BAD_REQUEST`, `UPSTREAM_ERROR`, `INTERNAL`). Validates origins against `ALLOWED_ORIGINS` / `ALLOWED_ORIGIN` or local hostnames with fallback to default allowed origin.
- **`_shared/payments-gateway.ts`**: Standardized payment RPC gateway seam (`createPendingPayment`, `submitPaymentProof`, `approvePaymentProof`, `rejectPaymentProof`, `markCashReceived`, `recordPaidPayment`).
...
| :--- | :--- | :--- |
| `event-dispatcher` | Internal / pg_cron | Drains `event_outbox` (WhatsApp, FCM push) |
...
1. **Zero Raw Table Business Mutations**: All financial, booking, and slot status state transitions are executed via `SECURITY DEFINER` SQL RPCs (`book_slot`, `apply_payment`, `cancel_booking`, `claim_event_outbox_batch`, `submit_payment_proof`, `approve_payment_proof`, `reject_payment_proof`, `mark_cash_received`).
...
3. **In-Memory Token Caching**: External OAuth2 tokens (FCM `expires_in - 60s`) are cached in memory to prevent rate-limiting and latency overhead.
4. **Role-Gated Diagnostics & Export**: Diagnostic and analytics export endpoints strictly require valid Bearer authentication with staff roles (`ADMIN`, `SUPER_ADMIN`).

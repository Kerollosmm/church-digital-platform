...
## 2. Functions Catalog (8 Functions)

All 8 functions disable gateway-level JWT verification in `supabase/config.toml` (`verify_jwt = false`) to enforce application-layer authentication and deterministic error payloads.
...
1. **Zero Raw Table Business Mutations**: All financial, booking, and slot status state transitions are executed via `SECURITY DEFINER` SQL RPCs (`book_slot`, `apply_payment`, `cancel_booking`, `claim_event_outbox_batch`).
...
5. **Role-Gated Diagnostics & Export**: Diagnostic and analytics export endpoints strictly require valid Bearer authentication with staff roles (`ADMIN`, `SUPER_ADMIN`).
